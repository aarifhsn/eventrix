...
try {
    ...
    if (!$bannerData) {
        $stmt = $pdo->prepare("INSERT INTO home_banners (subheading, heading, description, event_date) VALUES ('', '', '', '')");
        $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['home_banner_settings_form'])) {
    try {
        ...
        $stmt->execute([
            ...
            ':id' => $bannerData['id'] // This was the missing part - using banner ID instead of user ID
        ]);
